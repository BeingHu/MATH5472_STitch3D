...
import os
from networks_step2_sc import *


class Model():

    def __init__(self, adata_st, adata_basis, logit_step1, belong_matrix, feat_step1, counts_step1, libra_size_step1, # 加入 step1 的信息
                 hidden_dims=[512, 128],
                 n_heads=1,
                 slice_emb_dim=16,
                 coef_fe=0.1,
        ...
        # self.lY = torch.from_numpy(np.array(adata_st.obs["library_size"].values.reshape(-1, 1))).float().to(self.device) # 不用 cell level 的 libra size
        ...
        # step1 的信息
        self.logit_step1 = torch.from_numpy(logit_step1).float().to(self.device)
        self.belong_matrix = torch.from_numpy(belong_matrix.toarray()).float().to(self.device)
        self.feat_step1 = torch.from_numpy(feat_step1.toarray()).float().to(self.device)
        self.counts_step1 = torch.from_numpy(counts_step1).float().to(self.device) # 换成 spot level 的 counts
        self.libra_size_step1 = torch.from_numpy(libra_size_step1.reshape(-1, 1)).float().to(self.device) # 换成 spot level 的 library size
    ...
    def train(self, report_loss=True, step_interval=2000):
        ...
        for step in tqdm(range(self.training_steps)):
            loss = self.net(adj_matrix=self.A,
                            node_feats=self.X,
                            count_matrix=self.Y,
                            # library_size=self.lY, # 不用 cell level 的 libra size
                            slice_label=self.slice,
                            basis=self.basis,
                            logit_step1=self.logit_step1, # 加入 step1 的信息
                            belong_matrix=self.belong_matrix,
                            feat_step1=self.feat_step1,
                            counts_step1=self.counts_step1,
                            libra_size_step1=self.libra_size_step1)
    ...
    def eval(self, adata_st_list_raw, save=False, output_path="./results"):
        self.net.eval()
        self.Z, self.beta, self.alpha, self.gamma = self.net.evaluate(self.A, self.X, self.slice, self.logit_step1, self.belong_matrix) # 加入 step1 的信息
