...
class DeconvNet(nn.Module):
    ...
    def forward(self, 
        ...
        beta, alpha, logit_step1 = self.deconvolutioner(Z, slice_label_emb)
    ...
    def evaluate(self, adj_matrix, node_feats, slice_label):
        ...
        # deconvolutioner
        beta, alpha, logit_step1 = self.deconvolutioner(Z, slice_label_emb)  # 加入第一层的logit

        return Z, beta, alpha, logit_step1, self.gamma
    ...
    def deconvolutioner(self, Z, slice_label_emb):
        logit_step1 = self.deconv_beta_layer(F.elu(Z)) # 第一层的logit
        beta = F.softmax(logit_step1, dim=1)
        H = F.elu(torch.cat((Z, slice_label_emb), axis=1))
        alpha = self.deconv_alpha_layer(H)
        return beta, alpha, logit_step1
