...
class Model():
    ...
    def eval(self, adata_st_list_raw, save=False, output_path="./results"):
        self.net.eval()
        self.Z, self.beta, self.alpha, self.logit_step1, self.gamma = self.net.evaluate(self.A, self.X, self.slice) # 加入第一层的logit
        ...
        # 加入第一层的logit
        logit_step1 = self.logit_step1.detach().cpu().numpy()
        logit = pd.DataFrame(logit_step1)
        logit.index = self.adata_st.obs.index
        # self.adata_st.obsm['latent'] = cell_reps.loc[self.adata_st.obs_names, ].values
        if save == True:
            logit.to_csv(os.path.join(output_path, "logit_step1.csv"))
